...
export const ttlPlugin = (schema) => {
  ...
  const excludeExpiredDataFromFindQueries = async function (
    ...
    // here we will check is the record expired or not, 
    // if yes will update it with a new value.
    // if no will check is the record not exist at all will create new one.
